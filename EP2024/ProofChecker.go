...
func main() {
	...
	for i := 0; i < len(proofs); i++ {
		...
		go func(i int) {
			...
			message, err := base64.StdEncoding.DecodeString(proofs[i].Message)
			if err != nil {
				fmt.Print(err)
				os.Exit(1)
			}
			...
			if (us.Cmp(r) == 0) && (gs.Cmp(t) == 0) {
				passCount += 1
				// We remove the initial padding
				zeroIndex := slices.Index(message[1:], 0)
				message = message[zeroIndex+1:]
